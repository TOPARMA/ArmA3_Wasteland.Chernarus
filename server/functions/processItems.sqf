...
// Add items
{
  ...
  _attachmentsQty = if (count _x > 4) then { floor (_x select 4) } else { 0 }; // If item is weapon, quantity of magazines for each weapon (default = 0)
  _altMuzzleMagsQty = if (count _x > 5) then { floor (_x select 5) } else { 0 }; // If item is weapon, quantity of magazines for each weapon (default = 0)
      ...
      case "W": {
        ...
        if (typeName _class == "ARRAY") then {
          for "_i" from 1 to _quantity do {
            ...
            if(_attachmentsQty > 0) then {
              {
                _vehicle addItemCargoGlobal [_x, 1];
              } forEach ([_randomClass, _attachmentsQty] call randomAttachments);
            };

            if(_altMuzzleMagsQty > 0) then {
              {
                _vehicle addMagazineCargoGlobal [_x, 1];
              } forEach ([_randomClass, _attachmentsQty] call randomAltMuzzleMags);
            };
          };
        } else {
          ...
          if(_attachmentsQty > 0) then {
            {
              _vehicle addItemCargoGlobal [_x, 1];
            } forEach ([_class, _attachmentsQty] call randomAttachments);
          };

          if(_altMuzzleMagsQty > 0) then {
            {
              _vehicle addMagazineCargoGlobal [_x, 1];
            } forEach ([_class, _attachmentsQty] call randomAltMuzzleMags);
          };
        };
      };
      case "M": {
